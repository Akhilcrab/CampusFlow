...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Keep corresponding ExtractedItem isCompleted in sync!
    const placement = await db.placement.findUnique({ where: { id: placementId } });
    if (placement && placement.sourceReference) {
      await db.extractedItem.update({
        where: { id: placement.sourceReference },
        data: {
          isCompleted: placementStatus === 'Selected' || placementStatus === 'Rejected'
        }
